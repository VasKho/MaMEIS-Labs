...
func getExp(phi *big.Int) *big.Int {
	...
	for exponent = big.NewInt(2); exponent.Cmp(phi) == -1; exponent.Add(exponent, big.NewInt(1)) {
		if gcd(exponent, phi).Cmp(big.NewInt(1)) == 0 {
			break
		}
	}
	return exponent
...
func Create_sign(message string, key PrivateKey) SignedMessage {
	pub_key := PublicKey{key.D, key.N}
	sign := EncryptRSA(message, pub_key)
	return SignedMessage{message, sign}
}

func Check_sign(msg SignedMessage, key PublicKey) bool {
	prvt_key := PrivateKey{key.Exponent, key.N}
	return DecryptRSA(msg.Sign, prvt_key) == msg.Message
}
